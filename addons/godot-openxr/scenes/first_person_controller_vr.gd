...
@export var auto_initialise = true: set = set_auto_initialise
@export var enable_passthrough = false: set = set_enable_passthrough
@export (NodePath) var viewport : set = set_viewport
...
func _ready():
	...
	if auto_initialise && !Engine.is_editor_hint():
		initialise()
